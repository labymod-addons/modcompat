...
public abstract class MixinGameRendererMergeIris {
  ...
  private Matrix4f modcompat$applyBobbingSetViewMatrix(
    ...
    float i = Mth.lerp(f, this.minecraft.player.oPortalEffectIntensity,
        this.minecraft.player.portalEffectIntensity) * h * h;
    if (i > 0.0F) {
      int j = this.minecraft.player.hasEffect(MobEffects.NAUSEA) ? 7 : 20;
      float k = 5.0F / (i * i + 5.0F) - i * 0.04F;
      k *= k;
      Vector3f vector3f = new Vector3f(0.0F, Mth.SQRT_OF_TWO / 2.0F, Mth.SQRT_OF_TWO / 2.0F);
      float l = ((float) this.spinningEffectTime + f) * (float) j * (float) (Math.PI / 180.0);
